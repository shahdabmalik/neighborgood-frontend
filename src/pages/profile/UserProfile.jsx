...
const UserProfile = () => {
    ...
    return (
        <div className="w-full bg-white dark:bg-slate-950 transition-none md:transition-colors duration-300 ease-linear min-h-screen px-4 md:px-10" >
            ...
            {!isLoading ? <div className="max-w-screen-lg mx-auto pt-32 pb-24" >
                <div className="flex flex-col md:flex-row gap-10 md:gap-5" >
                    ...
                    <div className=" flex flex-col gap-4 px-4 pb-4 pt-2 flex-grow rounded-md transition-none md:transition-colors duration-300 ease-linear" >
                        ...
                        {!user?.shareDetails &&
                            <div className=" flex flex-col gap-4 text-slate-700 dark:text-slate-300 transition-none md:transition-colors duration-300 ease-linear" >
                                <div className="flex xs:gap-4 items-center" ><span className="font-semibold text-slate-800 dark:text-slate-200" >Phone:</span> <span className="flex-grow text-sm" >{user?.mobile}</span></div>
                                <div className="flex xs:gap-4 items-center" ><span className="font-semibold text-slate-800 dark:text-slate-200" >Email:</span> <span className="flex-grow text-sm" >{user?.email}</span></div>
                        ...
                        <div className=" mt-4" >
                            <button type="button" className="w-48 flex gap-2 justify-center rounded font-semibold text-black bg-primary-light hover:bg-primary transition-none md:transition-colors items-center border border-primary-light px-4 py-2" >Message <TbMessageCircle2Filled size={20} /></button>
                        </div>
                    </div>
                </div>
            ...
            </div> : <ProfileSkeleton userProfile={true} />}
        </div>
    )
}
